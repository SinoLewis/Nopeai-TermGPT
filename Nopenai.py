...
from gpt4all import gpt4all_query
from openai import openai_query
# from contexts import TERMINAL_COMMANDS
from sample import TERMINAL_COMMANDS
...
+ colorama.Fore.YELLOW + '--r [path/to/file.py]' + colorama.Fore.CYAN + '" to open and read some file into context (can do multiple files, and is embedded into some prompt.) Example: "Please change --r [mplexample.py] to be a dark theme"'+ colorama.Style.RESET_ALL)
    ...
    while not GPT_DONE:
        ...
        # reply_content = gpt4all.gpt4all_query(message_history=message_history, model="gpt-4")
        reply_content = openai.openai_query(message_history=message_history, model="gpt-4")
